<!-- resources/views/jenissurat/edit.blade.php -->
@extends('layouts.app')
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Data jenissurat</h3>
                    <a href="{{ route('jenissurat.index') }}" class="btn btn-sm btn-secondary float-right">Kembali</a>
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route('jenissurat.update', $jenissurat->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('jenis_surat') is-invalid @enderror" 
                                       ...
                                       value="{{ old('jenis_surat', $jenissurat->jenis_surat) }}" required>
                                @error('jenis_surat')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <textarea class="form-control @error('template') is-invalid @enderror"
                                    id="template" name="template" required>{{ old('template', $jenissurat->template) }}</textarea>
                                @error('template')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
